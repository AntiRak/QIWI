...
from data.config import QIWI_TOKEN, WALLET_QIWI, QIWI_P_PUB
        ...
        for transaction in transactions:
            if transaction.comment:
                if str(self.id) in transaction.comment:
                    if float(transaction.total.amount) >= self.amount:
                        return True
                    else:
...
@property
def invoice(self):
    link = f"https://oplata.qiwi.com/create?publicKey={QIWI_P_PUB}&amount={self.amount}&comment={self.id}"
